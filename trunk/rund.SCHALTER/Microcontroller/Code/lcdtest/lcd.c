...
#include "global.h"
//#include "timer.h"
#include "timerx8.h" /* S.L. timerx8.h statt timer.h für mega168 */
...
/* 
	S.L. Nachdem die sog. "E-Line" (enabled) bei uns auf einem anderen Port (PortB, Pin21) 
	liegt als der Rest der LCD-Pins (diese liegen auf LCD_CTRL_PORT, d.h. PortC), wurde in diesem 
	File folgendes geändert:
	Da die Funktionen cbi und sbi (clearBit und setBit, definiert in avrlibdefs.h) jeweils die 
	Portnummer bzw. das DataDirectionRegister (DDR) mit der Pinnummer Verknüpfen (&= bzw |=), 
	muss bei jedem ihrer Aufrufe mit LCD_CTRL_E als zweitem Übergabeparameter der erste 
	Übergabeparameter dahingehend geändert werden, dass er anstelle von LCD_CTRL_PORT bzw. 
	LCD_CTRL_DDR den tatsächlichen Port (PortB) bzw. das tatsächliche DDR (DDRB) referenziert. 
	
		--> #define SL_LCD_ENABLE_PORT	PORTB
		--> #define SL_LCD_ENABLE_DDR	DDRB
	

*/

/* S.L. Pin21 auf PortB (einziger LCD-Pin, der nicht auf PortC liegt!) ist für Enable des LCDs zuständig */
#define SL_LCD_ENABLE_PORT	PORTB
#define SL_LCD_ENABLE_DDR	DDRB
...
void lcdInitHW(void)
{
	...
	cbi(SL_LCD_ENABLE_PORT, LCD_CTRL_E);
	// initialize LCD control lines to output
	sbi(LCD_CTRL_DDR, LCD_CTRL_RS);
	sbi(LCD_CTRL_DDR, LCD_CTRL_RW);
	sbi(SL_LCD_ENABLE_DDR, LCD_CTRL_E);
	// initialize LCD data port to input
	// initialize LCD data lines to pull-up
	#ifdef LCD_DATA_4BIT
		outb(LCD_DATA_DDR, inb(LCD_DATA_DDR)&0xF0);		// SL! set data I/O lines to input (4bit)
		outb(LCD_DATA_POUT, inb(LCD_DATA_POUT)|0x0F);	// SL! set pull-ups to on (4bit)
	#else
...
void lcdBusyWait(void)
{
	...
	sbi(SL_LCD_ENABLE_PORT, LCD_CTRL_E);					// set "E" line
	LCD_DELAY;								// wait
	while(inb(LCD_DATA_PIN) & 1<<LCD_BUSY)  //SL! LCD_BUSY auf 3 statt 7
	{
		cbi(SL_LCD_ENABLE_PORT, LCD_CTRL_E);		// clear "E" line
		LCD_DELAY;									// wait
		LCD_DELAY;									// wait
		sbi(SL_LCD_ENABLE_PORT, LCD_CTRL_E);		// set "E" line
		LCD_DELAY;									// wait
...
// write the control byte to the display controller
	...
	cbi(LCD_CTRL_PORT, LCD_CTRL_RW);			// set R/W to "write"
		...
		// 4 bit write
		sbi(SL_LCD_ENABLE_PORT, LCD_CTRL_E);	// set "E" line
		outb(LCD_DATA_DDR, inb(LCD_DATA_DDR)|0x0F);	// SL! set data I/O lines to output (4bit)
		outb(LCD_DATA_POUT, (inb(LCD_DATA_POUT)&0xF0) | ((data&0xF0)>>4));	//SL!+shift um 4// output data, high 4 bits
		LCD_DELAY;								// wait
		LCD_DELAY;								// wait
		cbi(SL_LCD_ENABLE_PORT, LCD_CTRL_E);	// clear "E" line
		LCD_DELAY;								// wait
		LCD_DELAY;								// wait
		sbi(SL_LCD_ENABLE_PORT, LCD_CTRL_E);	// set "E" line
		outb(LCD_DATA_POUT, (inb(LCD_DATA_POUT)&0xF0) | (data&0x0F) );	//SL! output data, low 4 bits
		LCD_DELAY;								// wait
		LCD_DELAY;								// wait
		cbi(SL_LCD_ENABLE_PORT, LCD_CTRL_E);	// clear "E" line
	#else
	...
	//	leave data lines in input mode so they can be most easily used for other purposes	
		...
		outb(LCD_DATA_DDR, inb(LCD_DATA_DDR)&0xF0);		//SL! set data I/O lines to input (4bit)
		outb(LCD_DATA_POUT, inb(LCD_DATA_POUT)|0x0F);	//SL! set pull-ups to on (4bit)
		//SL! PINC leeren, damit das Busy-Flag nicht noch gesetzt sein kann
		outb(PINC,0x00);
	#else
		outb(LCD_DATA_DDR, 0x00);			// set data I/O lines to input (8bit)
		outb(LCD_DATA_POUT, 0xFF);			// set pull-ups to on (8bit)
